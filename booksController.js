...
const booksController = {
  ...
  async create(request, response, next) {
    ...
    response.status(201).json({ message: "Book has been added!" });
  },
  ...
  async delete(request, response, next) {
    ...
    response.status(202).json({ message: " Book has been deleted" });
  },
  ...
  async update(request, response, next) {
    ...
    const { rows } = await pool.query(
     ...
     title = $2 WHERE id = $3 RETURNING * `,
      [author, title, id]
    );
    response
      .status(201)
      .json({ message: "Book has been updated", book: rows[0] });
  },
};
